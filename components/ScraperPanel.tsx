...
export function ScraperPanel() {
  ...
  useEffect(() => {
    ...
    fetch("/api/scrape/fb-auth").then((r) => r.json()).then((d) => {
      setHasCookies(d.hasCookies ?? false);
    });
  ...
  const saveFbCookies = async () => {
    if (!fbCookies.trim()) return;
    await fetch("/api/scrape/fb-auth", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ cookieStr: fbCookies.trim() }),
    });
    setFbCookies("");
    setHasCookies(true);
    setCookieSaved(true);
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="rounded-2xl border border-gray-200 p-6 flex flex-col gap-4">
        ...
        <p className="text-sm text-gray-500">
          Paste Facebook group URLs (one per line). Facebook requires a session cookie to view group posts.
        </p>
        <details className="text-sm border border-gray-200 rounded-xl p-4">
          <summary className="cursor-pointer font-medium text-gray-700 hover:text-gray-900">
            Facebook session cookies{" "}
            {hasCookies && !cookieSaved && <span className="text-emerald-600 ml-2">✓ Saved</span>}
            {cookieSaved && <span className="text-emerald-600 ml-2">✓ Updated</span>}
          </summary>
          <div className="flex flex-col gap-3 mt-3">
            <ol className="text-xs text-gray-500 list-decimal ml-4 space-y-1">
              <li>Open <strong>facebook.com</strong> in Chrome/Firefox while logged in</li>
              <li>Press <strong>F12</strong> → Network tab → refresh the page</li>
              <li>Click any <strong>facebook.com</strong> request → Headers → find <strong>Cookie:</strong></li>
            ...
            <textarea
              ...
              className="w-full rounded-lg border border-gray-300 px-3 py-2 text-xs font-mono focus:outline-none focus:ring-2 focus:ring-emerald-500"
              dir="ltr"
            />
            <button
